...
function matches(table: any, term: string, pipe: PipeTransform) {
  if (table.studentName != undefined) {
    return table.studentName.toLowerCase().includes(term.toLowerCase())
        ;
  }
}
@Injectable({
...
export class AppointmentService {
  ...
  addAppointment(appointmentDTO:AppointmentDTO){
    ...
    return this.http.post(
        'http://localhost:9090/appointment/add-appointment',
        appointmentDTO,
        { headers: headers }
    );
  }
  ...
  getPendingAppointment(){
    ...
    return this.http.get(
        'http://localhost:9090/appointment/get-pending-appointment',
        { headers: headers }
    );
  }

  approveAppointment(appointmentDTO: AppointmentDTO) {
    const token = localStorage.getItem('token') || '';
    console.log('Token:', token);
    let headers = new HttpHeaders().append('Content-Type', 'application/json').append('Authorization','Bearer'+' '+token);
    console.log('Custom Headers:', headers);
    return this.http.post(
        'http://localhost:9090/appointment/approve-appointment',
        appointmentDTO,
        { headers: headers }
    );
  }

  getAppointmentDateVise(){
    const token = localStorage.getItem('token') || '';
    console.log('Token:', token);
    let headers = new HttpHeaders().append('Content-Type', 'application/json').append('Authorization','Bearer'+' '+token);
    console.log('Custom Headers:', headers);
    return this.http.get(
        'http://localhost:9090/appointment/get-appointments-date-vise',
        { headers: headers }
    );
  }

  getAppointmentDateViseAndUserVise(userId: string| null) {
    const token = localStorage.getItem('token') || '';
    console.log('Token:', token);
    let headers = new HttpHeaders().append('Content-Type', 'application/json').append('Authorization','Bearer'+' '+token);
    console.log('Custom Headers:', headers);
    return this.http.get(
        'http://localhost:9090/appointment/get-appointment-by-user?userID='+userId,
        { headers: headers }
    );
  }
}
