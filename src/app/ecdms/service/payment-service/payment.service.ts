...
function matches(table: any, term: string, pipe: PipeTransform) {
  if (table.type != undefined) {
    return table.type.toLowerCase().includes(term.toLowerCase())
        ;
  }
}
@Injectable({
...
export class PaymentService {
  ...
  getPayments(paymentFilterDTO:PaymentFilterDTO){
    ...
    return this.http.post(
        'http://localhost:9090/payment/get-payments',
        paymentFilterDTO,
        { headers: headers }
    );
  }

  makePayment(paymentDTO:PaymentDTO){
    const token = localStorage.getItem('token') || '';
    console.log('Token:', token);
    let headers = new HttpHeaders().append('Content-Type', 'application/json').append('Authorization','Bearer'+' '+token);
    console.log('Custom Headers:', headers);
    return this.http.post(
        'http://localhost:9090/payment/make-payment',
        paymentDTO,
        { headers: headers }
    );
  }
