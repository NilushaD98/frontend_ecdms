...
function matches(table: any, term: string, pipe: PipeTransform) {
  if (table.message != undefined) {
    return table.message.toLowerCase().includes(term.toLowerCase())
        ;
  }
}
...
export class NoticeService {
  ...
  getAllNotices(){
    ...
    return this.http.get(
        'http://localhost:9090/special_notice/get_all_special_notices',
        { headers: headers }
    );
  }

  removeSpecialNotice(id:number){
    const token = localStorage.getItem('token') || '';
    console.log('Token:', token);
    let headers = new HttpHeaders().append('Content-Type', 'application/json').append('Authorization','Bearer'+' '+token);
    console.log('Custom Headers:', headers);
    ...
    return this.http.delete(
        'http://localhost:9090/special_notice/remove_special-notice?noteID='+id,
        { headers: headers }
    );
  }

  addSpecialNotice(addSpecialNoticeDTO: AddSpecialNoticeDTO) {
    const token = localStorage.getItem('token') || '';
    console.log('Token:', token);
    let headers = new HttpHeaders().append('Content-Type', 'application/json').append('Authorization','Bearer'+' '+token);
    console.log('Custom Headers:', headers);
    ...
    return this.http.post(
        'http://localhost:9090/special_notice/add_special_notice',
        addSpecialNoticeDTO,
        { headers: headers }
    );
  }
  getSpecialNoticeUserVise(noticesRequestDTO:NoticesRequestDTO){
    const token = localStorage.getItem('token') || '';
    console.log('Token:', token);
    let headers = new HttpHeaders().append('Content-Type', 'application/json').append('Authorization','Bearer'+' '+token);
    console.log('Custom Headers:', headers);
    ...
    return this.http.post(
        'http://localhost:9090/special_notice/get_special_notice_user_vise',
        noticesRequestDTO,
        { headers: headers }
    );
  }
