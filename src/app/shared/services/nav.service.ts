...
export class NavService {
  private unsubscriber: Subject<void> = new Subject<void>(); // More specific type
  public screenWidth: BehaviorSubject<number> = new BehaviorSubject<number>(window.innerWidth);
  ...
  // For Horizontal Layout Mobile (if applicable, based on original)
  public horizontal: boolean = window.innerWidth >= 991; // Adjusted logic
  ...
  MENUITEMS_ADMIN: Menu[] = [
    ...
    { path: "/calender", title: "Calender", icon: "calender", type: "link" },
    {
      ...
      active: true,
      children: [
        { path: "/digital-portfolio/profile", title: "Digital Portfolio", type: "link" ,permission:['ADMIN','TEACHER','PARENT']},
        { path: "/digital-portfolio/add-post", title: "Add Announcement", type: "link" ,permission:['ADMIN','TEACHER','PARENT'] },
      ],
    ...
    },
    {
      ...
      active: false,
      children: [
        { path: "/attendance/student", title: "Attendance(Student)", type: "link",permission:['ADMIN','TEACHER','PARENT'] },
        { path: "/attendance/teacher", title: "Attendance(Teacher)", type: "link",permission:['ADMIN','TEACHER','PARENT'] },
      ],
      permission:['ADMIN','TEACHER','PARENT']
    },
    {
      ...
      active: false,
      children: [
        { path: "/academic-milestones/tests", title: "Tests", type: "link" ,permission:['ADMIN','TEACHER','PARENT']},
        { path: "/academic-milestones/exam-results", title: "Exam Results", type: "link" ,permission:['ADMIN','TEACHER','PARENT']},
      ],
      permission:['ADMIN','TEACHER','PARENT']
    },
    {
      ...
      active: false,
      children: [
        { path: "/appointment/appointment", title: "Appointment", type: "link" ,permission:['ADMIN','TEACHER','PARENT']},
      ],
      permission:['ADMIN','TEACHER','PARENT']
    },
    {
      ...
      active: false,
      children: [
        { path: "/payments/payment", title: "Payment Registration", type: "link" ,permission:['ADMIN','TEACHER','PARENT']},
      ],
      permission:['ADMIN','TEACHER','PARENT']
    },
    {
      ...
      active: false,
      children: [
        { path: "/student-management/manage-student", title: "Students", type: "link" ,permission:['ADMIN','TEACHER','PARENT']},
        { path: "/student-management/manage-teacher", title: "Teachers", type: "link" ,permission:['ADMIN','TEACHER','PARENT']},
      ],
      permission:['ADMIN','TEACHER','PARENT']
    },
    {
  ...
  MENUITEMS_TEACHER:Menu[]=[

    { path: "/profile/teacher", title: "Profile", icon: "calender", type: "link" },
    { path: "/calender", title: "Calender", icon: "calender", type: "link" },
    {
      title: "Digital Portfolio",
      icon: "users",
  ...
  MENUITEMS_PARENTS:Menu[]=[

    { path: "/profile/student", title: "Profile", icon: "calender", type: "link" },
    { path: "/calender", title: "Calender", icon: "calender", type: "link" },
    {
      title: "Digital Portfolio",
      icon: "users",
  ...
  constructor(private router: Router) {
    ...
    const userRole = localStorage.getItem('user_type'); // e.g., 'ADMIN', 'TEACHER', 'PARENT'

    let menuItems: Menu[] = this.MENUITEMS_ADMIN; // default fallback
    console.log(userRole)
    if (userRole == 'TEACHER') {
      menuItems = [];
      menuItems = this.MENUITEMS_TEACHER;
    } else if (userRole =='PARENT') {
      menuItems = [];

      menuItems = this.MENUITEMS_PARENTS;
    } else if (userRole == 'ADMIN') {
      menuItems = [];
      menuItems = this.MENUITEMS_ADMIN;
    } else {
      console.warn('Unknown or missing role:', userRole);
      // Optionally redirect to login if role is missing/unexpected
      // this.router.navigate(['/authentication/login']);
    }

    // 📡 Emit the selected menu
    ...
    fromEvent(window, 'resize')
        ...
        .subscribe((evt: Event) => { // More specific type than 'any'
          ...
          if (target.innerWidth < 991) {
            ...
            // Optional: Expand sidebar when resizing to large screen?
    ...
    if (window.innerWidth < 991) {
      this.router.events
          ...
          .subscribe(() => {
            this.collapseSidebar = true; // Collapse after navigation on mobile
          });
    }
  }
  ...
  // Call this method when the service is destroyed to prevent memory leaks
  ngOnDestroy(): void {
    this.unsubscriber.next();
    this.unsubscriber.complete();
  }

  // Method to manually toggle the sidebar collapse state
  toggleSidebar(): void {
    this.collapseSidebar = !this.collapseSidebar;
  }

  // Method to manually toggle full screen mode (implementation depends on browser APIs)
  toggleFullScreen(): void {
    // Basic full screen toggle logic (might need vendor prefixes for full support)
    if (!this.fullScreen) {
      const elem = document.documentElement;
      if (elem.requestFullscreen) {
        elem.requestFullscreen().then(() => this.fullScreen = true);
      } else if ((elem as any).mozRequestFullScreen) { /* Firefox */
        (elem as any).mozRequestFullScreen().then(() => this.fullScreen = true);
      } else if ((elem as any).webkitRequestFullscreen) { /* Chrome, Safari & Opera */
        (elem as any).webkitRequestFullscreen().then(() => this.fullScreen = true);
      } else if ((elem as any).msRequestFullscreen) { /* IE/Edge */
        (elem as any).msRequestFullscreen().then(() => this.fullScreen = true);
      }
    } else {
      if (document.exitFullscreen) {
        document.exitFullscreen().then(() => this.fullScreen = false);
      } else if ((document as any).mozCancelFullScreen) { /* Firefox */
        (document as any).mozCancelFullScreen().then(() => this.fullScreen = false);
      } else if ((document as any).webkitExitFullscreen) { /* Chrome, Safari & Opera */
        (document as any).webkitExitFullscreen().then(() => this.fullScreen = false);
      } else if ((document as any).msExitFullscreen) { /* IE/Edge */
        (document as any).msExitFullscreen().then(() => this.fullScreen = false);
      }
    }
  }
